...
def calculate_totals(transactions: List[Transaction]) -> Tuple[float, float, float]:
    ...
    :param transactions: 거래(Transaction, Income, Expense) 객체들의 리스트
    :return: (총 수입, 총 지출, 순 잔액) 형태의 튜플

    >>> from my_package.subclass import Income, Expense
    >>> txs = [Income(5000.0, "용돈", "2026-06-13"), Expense(2000.0, "식비", "2026-06-13")]
...
def get_category_distribution(transactions: List[Transaction]) -> Dict[str, float]:
    """주어진 거래 목록의 카테고리별 지출 분포를 계산합니다. (수입은 집계에서 제외하고 지출만 계산)

    :param transactions: 거래 객체들의 리스트
    :return: 카테고리명을 키로, 지출 합계를 값으로 하는 딕셔너리

    >>> from my_package.subclass import Expense
    >>> txs = [Expense(1500.0, "식비", "2026-06-13"), Expense(3000.0, "교통비", "2026-06-13"), Expense(4500.0, "식비", "2026-06-13")]
    >>> get_category_distribution(txs)
    {'식비': 6000.0, '교통비': 3000.0}
    """
    ...
    for tx in transactions:
        ...
        if tx_dict.get("type") == "expense":
            ...
            distribution[category] = distribution.get(category, 0.0) + tx.amount
